...
def create_graphic_features(tf: TFBS, genes: list[Gene]) -> list[GraphicFeature]:
    features=[]
    features.append(
        GraphicFeature(start=tf.start, end=tf.end, strand=eval(tf.strand.value + str(tf.start % 3 or 3)), color=TFBS_COLOUR, label="TF")
    )
    for gene in genes:
        features.append(
...
def visualize_operons(tf_genes: dict[TFBS, list[Gene]]):
    ...
    for tf, genes in tf_genes.items():
        ...
        ax.figure.savefig(str(Path() / 'output' / f'{tf.score}.png'), dpi = 200)
